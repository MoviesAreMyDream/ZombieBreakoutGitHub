...
public class KinectModelControllerV2 : MonoBehaviour {
    ...
    void Start()
    {
        ...
        _baseRotation = new Quaternion[(int)Body.JointCount];
        _boneDir = new Vector3[(int)Body.JointCount];
        ...
        for (int ii = 0; ii < (int)Body.JointCount; ii++)
        {
            if ((_nullMask & (uint)(1 << ii)) <= 0)
            {
                ...
                if (ii % 4 == 3 && ((_nullMask & (uint)(1 << (ii / 4) + (int)Body.JointCount)) <= 0))
                {
                    _boneDir[ii] = _bones[(ii / 4) + (int)Body.JointCount].transform.position - _bones[ii].transform.position;
                }
        ...
        _chestRight -= Vector3.Project(_chestRight, _boneDir[(int)JointType.SpineBase]);
    ...
    void RotateJoint(int bone)
    {
        ...
        if (bone == (int)JointType.SpineBase)
        {
        ...
        if (bone % 4 == 0)
        {
            ...
            if (bone == (int)JointType.HipLeft)
            {
               ...
               // target = (int)JointType.HipLeft + (int)JointType.HipRight / 2F - (int)JointType.SpineBase);
            }
            //otherwise it is one of the shoulders
            else
            {
                //target = vector from shoulder_center to bone
               // target = [bone - (int)JointType.SpineShoulder];
            }
        }
        else
        {
        ...
        target = transform.TransformDirection(0,0,0);
        ...
        if (bone == (int)JointType.SpineBase)
        {
            ...
            dir = _chestRight;
            //target = [(int)JointType.ShoulderRight - (int)JointType.ShoulderLeft];
        ...
        else if (bone == (int)JointType.HipLeft)
        {
            ...
            dir = _hipRight;
            //target = [(int)JointType.HipRight - (int)JointType.HipLeft];
